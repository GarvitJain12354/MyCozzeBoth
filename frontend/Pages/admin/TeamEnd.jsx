...
import { getAllUser } from "../../store/Action/Admin";
...
const Tenant = () => {
  ...
  const { allUser, totalPages } = useSelector((state) => state.Admin); // Adjust according to your state structure
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10; // Number of rows per page
  const [id, setid] = useState("");
  useEffect(() => {
    dispatch(getAllUser(currentPage, itemsPerPage));
  ...
  return (
    <div className="w-full h-screen overflow-hidden">
      ...
      <div className="flex w-full h-[89vh] overflow-hidden translate-y-20">
        <AdminSidebar title={"Tenant"} isOpen={isOpen}></AdminSidebar>

        <div className="w-full p-10 overflow-y-auto h-full">
          <div className="py-5">
            <h1 className="text-3xl max-md:text-xl">Tenant</h1>
          </div>

          <GraphPage role={"flatemate"}/>
